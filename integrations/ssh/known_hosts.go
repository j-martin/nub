...
// Taken from https://github.com/coreos/fleet/blob/46614bece93762d3fd35ed9c7a1ded4768080c0a/ssh/known_hosts.go
...
func (kc *HostKeyChecker) GetHostKeyAlgorithms(addr string) []string {
	...
	if err != nil {
		log.Printf("Failed to parse address %v: %v", addr, err)
		return nil
	}
	...
	for pattern, keys := range hostKeys {
		if !matchHost(remoteAddr, pattern) {
			...
			if err != nil {
				log.Printf("Failed to resolve TCP address %v: %v", addr, err)
				continue
			}
			ipAddr, err := kc.addrToHostPort(remoteIP.String())
			if err != nil {
				log.Printf("Failed to parse address %v: %v", remoteIP.String(), err)
				continue
			}
...
func (kc *HostKeyChecker) addrToHostPort(a string) (string, error) {
	...
	if err != nil {
		log.Printf("Unable to parse addr %s: %v", a, err)
		return "", err
	}
...
func NewHostKeyFile() *HostKeyFile {
	currentUser, err := user.Current()
	if err != nil {
		log.Fatalf("Failed to get currentUesr path.")
	}
	return &HostKeyFile{path.Join(currentUser.HomeDir, ".ssh", "known_hosts")}
}
